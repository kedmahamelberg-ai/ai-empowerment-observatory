...
const STORAGE_KEY = "aieo_event_active_learning_v1";
...
function sourceCard(article, side) {
  const translated =
    article.english_headline &&
    article.english_headline !== article.original_headline;

  const snippet = article.snippet
    ? `<p class="snippet">${escapeHTML(article.snippet)}</p>`
    : "";
  ...
  return `
    <section class="article">
      <span class="label">Article ${side}</span>
      <h2>${escapeHTML(article.original_headline)}</h2>
      ...
      <div class="article-footer">
        <span>
          ...
          · ${escapeHTML(article.source_language.toUpperCase())}
        </span>
        ${source}
      </div>
    </section>
...
function render() {
  summaryEl.innerHTML = `
    <strong>${escapeHTML(payload.meta.sample_size)} high-information pairs</strong>
    selected by active learning from
    ${escapeHTML(payload.meta.candidate_pool_size)} candidates.
    Model predictions are hidden while you label.
  `;
  ...
  pairsEl.innerHTML = payload.pairs
    .map((pair, index) => {
      ...
      const choices = [
        ...
        ["unclear_from_headlines", "Unclear from headlines"]
      ]
        .map(([value, title]) => `
      ...
      return `
        <article class="pair-card">
          ...
          <div class="article-grid">
            ${sourceCard(pair.article_a, "A")}
            ${sourceCard(pair.article_b, "B")}
          </div>
...
function downloadCSV() {
  ...
  for (const pair of payload.pairs) {
    rows.push([
      ...
      pair.current_same_cluster,
      pair.selection_reasons.join("|"),
      pair.information_score
    ]);
  }
  ...
  anchor.href = url;
  anchor.download = "aieo-event-identity-active-learning-labels.csv";
...
async function init() {
  ...
  try {
    ...
    clearBtn.addEventListener("click", () => {
      if (!confirm("Clear all active-learning labels in this browser?")) {
        return;
      }
    ...
    summaryEl.innerHTML = `
      <strong>No active-learning sample yet.</strong>
      Run “Generate Event Identity Active Learning” in GitHub Actions.
    `;
  }
}
