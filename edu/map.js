...
const elements = {
  ...
  openStories: document.getElementById("open-stories"),
  storiesDialog: document.getElementById("stories-dialog"),
  closeStories: document.getElementById("close-stories")
};
...
function attachInterfaceEvents() {
  ...
  elements.openLearn.addEventListener("click", () => {
    elements.learnDialog.showModal();
  });

  elements.closeLearn.addEventListener("click", () => {
    elements.learnDialog.close();
  });

  elements.learnDialog.addEventListener("click", (event) => {
    if (event.target === elements.learnDialog) {
      elements.learnDialog.close();
    }
  });

  elements.openStories.addEventListener("click", () => {
    elements.storiesDialog.showModal();
  });

  elements.closeStories.addEventListener("click", () => {
    elements.storiesDialog.close();
  });

  elements.storiesDialog.addEventListener("click", (event) => {
    if (event.target === elements.storiesDialog) {
      elements.storiesDialog.close();
    }
  });
}
