import {OUTCOMES, escapeHTML as e, formatRange, fetchJSON, weeklyModel, weeklyTakeaway, outcomeFor} from '../public-data.js?v=7.3.0';
import {setupNavigation, renderEvidenceContext, renderStories, sortEvents, eventMarkets, countingDetail} from '../site.js?v=7.3.0';
...
let auditMode=params.get('scope')==='audit' || signal==='too_little_evidence';
let limit=12;
...
function render() {
  if (signal==='too_little_evidence') auditMode=true;
  const source = auditMode ? release : model.analysisRelease;
  coverage=new Map((source?.units?.coverage_articles || []).map(a=>[String(a.article_id),a]));
  const events=sortEvents(source?.evidence || []).filter(event=>(!market || eventMarkets(event,coverage).includes(market)) && matchesNovelty(event) && (signal==='all' || (signal==='too_little_evidence' ? model.auditRecords?.get(String(event.effective_event_id || event.event_id))?.eligible===false : model.ready && outcomeFor(model.rows.get(String(event.effective_event_id || event.event_id)))===signal)));
  renderStories(el('evidence-list'),events.slice(0,limit),model,markets,coverage);
  el('evidence-scope').textContent=`${events.length} of ${auditMode ? model.inventory?.total || 0 : model.total} ${auditMode ? 'collected developments · audit inventory' : 'developments with complete source content'}${signal!=='all' ? ` · ${signal==='too_little_evidence' ? 'Excluded from analysis' : OUTCOMES[signal]}`:''}${market ? ` · ${markets[market]?.name || market} search`:''} · Showing ${Math.min(limit,events.length)}`;
  el('show-more-evidence').hidden=events.length<=limit;
  if (model.ready) el('count-detail').innerHTML=countingDetail(auditMode ? model.inventory : model);
  ...
  for(const [key,value] of [['scope',auditMode?'audit':''],['signal',signal==='all'?'':signal],['market',market],['view',novelty==='all'?'':novelty]]){if(value)url.searchParams.set(key,value);else url.searchParams.delete(key);}
...
async function init() {
  ...
  try {
    ...
    coverage=new Map(((auditMode ? release : model.analysisRelease)?.units?.coverage_articles || []).map(a=>[String(a.article_id),a]));
    ...
    el('news-context').innerHTML=`${e(weeklyTakeaway(model))} <a href="/">See the weekly overview →</a><br><a href="/edu/#evidence">Complete-content developments</a> · <a href="/edu/?scope=audit#evidence">All collected developments and exclusions</a>`;
