...
function range(a,b){ const start=parseDate(a),end=parseDate(b); if(!start||!end)return "—"; const same=start.getUTCMonth()===end.getUTCMonth()&&start.getUTCFullYear()===end.getUTCFullYear(); return same?`${start.getUTCDate()}–${dateLong.format(end)}`:`${dateLong.format(start)}–${dateLong.format(end)}`; }
...
async function init(){
  try{
    ...
    setText("method-through",time(current.data_current_through||current.generated_at)||"—");
